...
void refreshDirtyStatistics(Database db) {
  final pending = db.select('SELECT node_id,revision FROM index_stats_dirty');
  final now = DateTime.now().millisecondsSinceEpoch;
  for (final row in pending) {
    final id = row['node_id'];
    db.execute(
        '''INSERT INTO index_node_stats(node_id,direct_entity_count,descendant_entity_count,
      child_node_count,updated_at,revision)
      WITH RECURSIVE subtree(id) AS (
        SELECT id FROM index_nodes WHERE id=?
        UNION SELECT n.id FROM index_nodes n JOIN subtree p ON n.parent_id=p.id
      ) SELECT ?,
        (SELECT COUNT(*) FROM index_node_entities l JOIN entities e ON e.id=l.entity_id WHERE l.index_node_id=? AND e.archived=0),
        (SELECT COUNT(DISTINCT e.id) FROM index_node_entities l JOIN entities e ON e.id=l.entity_id
          WHERE l.index_node_id IN(SELECT id FROM subtree) AND e.archived=0),
        (SELECT COUNT(*) FROM index_nodes WHERE parent_id=?),?,?
      WHERE EXISTS(SELECT 1 FROM index_nodes WHERE id=?)
      ON CONFLICT(node_id) DO UPDATE SET direct_entity_count=excluded.direct_entity_count,
        ...
        [id, id, id, id, now, row['revision'], id]);

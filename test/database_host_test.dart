...
void main() {
  test('host serializes commands, rolls back batches and drains before close', () async {
    final directory = await Directory.systemTemp.createTemp('database_host_');
    addTearDown(() => directory.delete(recursive: true));
    final host = await DatabaseHost.start(databasePath: p.join(directory.path, 'library.db'));
    addTearDown(host.close);
    final library = LibraryClient(host);
    final root = await library.ensureDirectoryIndexRoot(p.join(directory.path, 'source'));
    ...
    await expectLater(host.executeBatch(const [
  ...
  test('startup failure completes instead of waiting indefinitely', () async {
    final directory = await Directory.systemTemp.createTemp('database_host_bad_');
    addTearDown(() => directory.delete(recursive: true));
    await expectLater(
      DatabaseHost.start(databasePath: directory.path).timeout(const Duration(seconds: 15)),

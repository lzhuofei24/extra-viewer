...
void main() {
  test('software frame keeps aspect ratio and target pixel budget', () {
    final bytes =
        Uint8List.fromList(img.encodePng(img.Image(width: 1600, height: 900)));
    final result = prepareVideoThumbnailPixels(bytes);
  ...
  test('invalid capture is an error rather than a successful empty preview',
      () {
    expect(
        () => prepareVideoThumbnailPixels(Uint8List(0)), throwsFormatException);
  });
}

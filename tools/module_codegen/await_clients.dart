...
void main(List<String> paths) {
  ...
  for (final path in paths) {
    ...
    unit.accept(visitor);
    var result = content;
    for (final edit in (visitor.edits.values.toList()..sort((a,b) => b.start.compareTo(a.start)))) {
...
class _Edit { _Edit(this.start, this.end, this.value); final int start, end; final String value; }
class _AwaitClients extends RecursiveAstVisitor<void> {
  final edits = <String, _Edit>{};
  void add(int start, int end, String value) => edits['$start:$end'] = _Edit(start,end,value);
  @override void visitMethodInvocation(MethodInvocation node) {
    super.visitMethodInvocation(node);
    if (!{'library','builds','repository','widget.repository','_repository'}.contains(node.target?.toSource())) return;
    if (node.parent is AwaitExpression) return;
    FunctionBody? body;
    for (AstNode? parent = node.parent; parent != null; parent = parent.parent) {
      if (parent is FunctionBody) { body = parent; break; }
    }
    if (body == null || body.parent is ConstructorDeclaration) return;
    if (body.parent case MethodDeclaration m when m.name.lexeme == 'build' || m.isGetter) return;
    add(node.offset,node.offset,'(await '); add(node.end,node.end,')');
    if (!body.isAsynchronous) {
      ...
      if (declaration is MethodDeclaration && declaration.returnType != null) {
        ...
        if (!type.toSource().startsWith('Future')) add(type.offset,type.end,'Future<${type.toSource()}>');
      }
    }
  }
}

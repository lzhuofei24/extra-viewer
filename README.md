...
Extra Viewer 是一款 **Android 本地资料浏览器**。文件始终保留在原目录中，你可以按原始目录浏览，也可以用分类重新整理。应用会生成预览并保存阅读和播放进度。
...
- **中文友好**：支持目录与分类搜索、中文短词匹配、完整路径展示和 Android SAF 目录授权。
...
按名称查找目录或分类。搜索支持中文短词、类型筛选和完整路径，不读取文件正文，也不会加载整个资料库的预览图。
...
Extra Viewer 目前处于 Android 内测阶段。项目重点是本地资料管理、长任务恢复和大规模媒体浏览；搜索只覆盖目录和分类名称，不搜索文件名、文件正文或云端内容。

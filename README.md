...
Extra Viewer 是一款 **Android 本地优先资料浏览器**。它把真实文件保持在原位置，通过目录索引、树索引和图索引组织内容；应用负责建立实体、生成预览、记录阅读与播放进度，让大量图片、视频、音频和电子书都能用同一种方式浏览。
...
- **一次扫描，多种组织方式**：目录索引保留原始层级，树索引用于自由整理，图索引用节点和关系表达知识结构。
...
把来自不同目录的实体加入同一个树索引，建立自己的阅读专题、收藏主题或工作集合。树索引只保存引用，不复制原始文件。
...
通过节点搜索查找目录索引、树索引和图索引中的节点。搜索支持中文子串、类型筛选、完整面包屑和分页结果，不读取实体正文，也不会因为搜索加载整库缩略图。
